...
공개 예시본입니다. 실제 볼트는 비공개이며, 이 저장소에는 구조와 운용 절차만 옮겨
담았습니다.

> **이 저장소의 값은 전부 자리표시자입니다.** 키와 비밀번호, 토큰은 물론이고 도메인과
> 호스트명도 실제 값이 아닙니다.
...
이 볼트는 git-crypt로 암호화한 상태로만 커밋하고, 복호화 키는 저장소 밖 오프라인에 둡니다. 커밋에는 암호문만 들어가므로 원격이
...
`export-key`로 뽑은 키는 저장소 밖으로 나가야 합니다. `.git/git-crypt/keys/default`에도
